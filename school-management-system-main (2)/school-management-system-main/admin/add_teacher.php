...
<div class="page-header">
    <div class="d-flex align-items-center gap-3">
        ...
        <div><h4><i class="bi bi-person-plus-fill me-2"></i>Add New Teacher</h4><p>Fill in the details — credentials are generated automatically.</p></div>
    </div>
</div>
...
<div class="omk-card mb-4">
    <div class="card-body p-4">
        <div class="d-flex align-items-center gap-4">
            <div class="d-flex align-items-center gap-3">
                <div style="width:56px;height:56px;border-radius:16px;background:linear-gradient(135deg,#3b82f6,#1d4ed8);display:flex;align-items:center;justify-content:center">
                    <i class="bi bi-person-video3 text-white fs-4"></i>
                </div>
                <div>
                    <div class="text-muted small fw-600 text-uppercase" style="letter-spacing:1px">Auto-Generated Teacher ID</div>
            ...
            <div class="ms-auto"><span class="badge bg-primary-subtle text-primary px-3 py-2"><i class="bi bi-shield-check me-1"></i>Unique & Auto-assigned</span></div>
        </div>
    </div>
</div>
...
<div class="card-header d-flex align-items-center gap-2">
    ...
    <span class="fw-semibold">Teacher Information</span>
</div>
<div class="card-body p-4">
<form id="userForm">
    ...
    <div class="d-flex gap-2 mt-4 flex-wrap">
        <button type="submit" class="btn btn-primary px-4" id="submitBtn"><i class="bi bi-person-check me-2"></i>Add Teacher</button>
        <button type="button" class="btn btn-outline-secondary" onclick="document.getElementById('userForm').reset();ef.dataset.manual='';pf.dataset.manual='';"><i class="bi bi-arrow-counterclockwise me-1"></i>Reset</button>
        <a href="users.php" class="btn btn-link text-muted ms-auto">Cancel</a>
    </div>
</form>
<div id="successPanel" class="d-none mt-4 p-4 rounded-3" style="background:#eff6ff;border:1px solid #93c5fd">
    <div class="d-flex align-items-start gap-3">
        <i class="bi bi-check-circle-fill text-primary fs-3 mt-1"></i>
        <div class="flex-grow-1">
            <div class="fw-700 text-primary mb-2">Teacher Added Successfully!</div>
            ...
            <div class="d-flex gap-2">
                <button class="btn btn-primary btn-sm" onclick="addAnother()"><i class="bi bi-plus me-1"></i>Add Another</button>
                <a href="users.php" class="btn btn-outline-primary btn-sm"><i class="bi bi-people me-1"></i>View All Users</a>
                <a href="courses.php" class="btn btn-outline-success btn-sm"><i class="bi bi-link me-1"></i>Assign Courses</a>
            </div>
        </div>
    </div>
</div>
...
let sysId='';
fetch(`${BASE}/api/admin_ajax.php?action=get_next_id&role=teacher`).then(r=>r.json()).then(d=>{sysId=d.id;document.getElementById('displayId').textContent=d.id;});
...
document.getElementById('userForm').addEventListener('submit',async function(e){
    ...
    try{const r=await fetch(`${BASE}/api/admin_ajax.php`,{method:'POST',body:fd});const d=await r.json();
        if(d.success){document.getElementById('sucName').textContent=d.name;document.getElementById('sucId').textContent=d.system_id;document.getElementById('sucEmail').textContent=d.email;document.getElementById('successPanel').classList.remove('d-none');document.getElementById('userForm').classList.add('d-none');toast('Teacher added!','success');fetch(`${BASE}/api/admin_ajax.php?action=get_next_id&role=teacher`).then(r=>r.json()).then(d=>{sysId=d.id;document.getElementById('displayId').textContent=d.id;});}else{toast(d.error,'danger');}
    }catch(err){toast('Server error.','danger');}
    btn.innerHTML='<i class="bi bi-person-check me-2"></i>Add Teacher';btn.disabled=false;
});
